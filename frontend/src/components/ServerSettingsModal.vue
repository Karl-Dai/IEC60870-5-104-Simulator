      ...
      <div v-if="visible" class="settings-backdrop dialog-blur">
        <div class="settings-modal" role="dialog" aria-modal="true" aria-labelledby="server-settings-title" :aria-busy="busy">
          <header>
            ...
            <AppButton variant="ghost" icon="x" :disabled="mutating" :aria-label="t('common.cancel')" @click="close" />
          </header>
          <div class="settings-body">
            ...
            <div v-if="errorText" class="settings-error" role="alert">
              <p>{{ errorText }}</p>
              <button v-if="!ready" type="button" class="secondary" :disabled="busy" @click="load">{{ t('serverSettings.reload') }}</button>
            </div>
          </div>
          <footer>
            <span class="hint">{{ t('serverSettings.saveHint') }}</span>
            <AppButton class="secondary" :disabled="mutating" @click="close">{{ t('common.cancel') }}</AppButton>
            <AppButton variant="primary" class="primary" :disabled="locked || !dirty" @click="save">
...
.running-notice { padding: 12px; margin-bottom: 16px; border: 1px solid var(--warning); border-radius: 5px; }
.running-notice p { margin: 0 0 10px; color: var(--warning); font-size: 12px; line-height: 1.5; }
...
footer .hint { flex: 1; }
.settings-error { margin-top: 14px; padding: 10px; color: var(--danger); background: color-mix(in srgb, var(--danger) 10%, transparent); border-radius: 4px; font-size: 12px; white-space: pre-wrap; overflow-wrap: anywhere; }
