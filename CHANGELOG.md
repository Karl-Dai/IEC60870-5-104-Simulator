...
- 从站支持导入 JSON，按定时规则改变点位值 / Import JSON to change slave point values on a schedule (#88)
...
- 树节点选中态改为深蓝底与高对比度前景色，提升服务器、站和分类的可读性 / Improve selected tree row contrast and readability
...
- ui: 补齐表头全选和连接配置快捷编辑 (#83)
- TLS：兼容 X.509 v1/v3 证书，支持创建后编辑或关闭 TLS，失败时保留表单 / Support X.509 v1/v3 certificates and editable server TLS settings

### Fixed 修复

- 主站手动断开后停止自动重连 / Stop automatic reconnect after manual disconnect

### Changed 改进

- 发布构建为 Ubuntu 依赖安装增加超时限制 (#82) / Bound Ubuntu dependency installation time (#82)
...
- 修复子站数据点详情面板在窄宽度下的文本重叠、字段裁切和写入按钮溢出问题 / Fix Point Details overlap, clipping, and write-action overflow at narrow widths (#81)
...
- 统一主站与子站的点位浏览、多选操作和双点状态图例；新建或加载配置后可直接查看各类型点位数量。
- TLS 配置支持通过系统文件选择器选择证书和私钥：子站覆盖服务器证书、服务器私钥与 CA，主站覆盖 CA、客户端证书与客户端私钥；保留手工输入并按文件类型过滤。
...
- 主站连接工作区自动持久化：新增、删除、编辑、加载配置及自动发现公共地址后立即保存，重启时恢复全部连接以及 TLS、SOCKS5 和协议参数。

### Fixed 修复

- 增加启动恢复屏障和并发保存锁，避免多连接恢复、修改和保存之间的竞态与旧快照覆盖。

### Changed 改进

- 恢复后的连接默认为未连接；自动持久化省略庞大的实时点表，手动“保存配置”仍保留点表快照。
