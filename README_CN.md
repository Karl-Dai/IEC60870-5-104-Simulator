...
当前从站前端已在工具栏直接提供**导入 CSV / 导出 CSV / 下载模板**。展开通信日志后,可按方向、帧类型或 Type ID 筛选,搜索解析详情和原始报文,查看当前/总条数,并导出当前筛选结果。

![从站 CSV 点表与可搜索报文日志](docs/screenshots/slave-point-csv-log-analysis.png)

**从站 · 实时查看主站连接**

**v1.15.6** 新增:每个从站服务器节点都会实时显示已连接主站数量。点击数量徽标,或在服务器右键菜单选择**查看主站连接**,即可查看各主站的 IP/端口,以及 IEC 104 数据传输是否已通过 STARTDT 激活。主站接入、激活传输或断开时,数量和详情列表都会自动刷新。
...
![从站有界随机仿真](docs/screenshots/slave-random-simulation.png)
...
手动安装请使用上表安装包。macOS 的 `.app.tar.gz` 及其 `.sig` 是安装 `.dmg` 后供应用内自动更新使用的签名内部产物,用户无需手动下载或解压。
...
该教程图取自 v1.15.6 标签之后的当前 `main` 前端,并非 v1.15.6 发行包。下一次版本号提升前,界面仍显示 `v1.15.6`;这张 1600×900 截图完整保留了标签后新增的 CSV 点表操作、**模拟设置**入口、控制点用途提示、活动随机模拟标识和数据点总数。
